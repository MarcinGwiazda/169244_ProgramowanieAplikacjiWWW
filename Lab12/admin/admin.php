...
function Nawigacja() {
    echo '<nav>
			...
			<a href="admin.php?action=cart" class="button">Pokaż Koszyk</a>
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<tr>
            ...
            <td>
                ...
                <a href='?action=delete_product&id={$row['id']}'>Usuń</a><br><br>";

        // Formularz dodawania do koszyka z możliwością wyboru ilości
        echo "<form method='GET' action=''>
                ...
                <label for='quantity_{$row['id']}'>Ilość:</label>
                <input type='number' id='quantity_{$row['id']}' name='quantity' value='1' min='1' max='{$row['ilosc_magazyn']}' style='width: 50px;'><br><br>
...
function addToCart($productId, $quantity, $cena_netto, $vat) {
    ...
    // Obliczenie całkowitej ceny netto dla wybranej ilości
    $cena_netto_calkowita = $cena_netto * $quantity;

    // Obliczenie całkowitej ceny brutto dla wybranej ilości
    $cena_brutto_calkowita = $cena_netto_calkowita * (1 + $vat / 100);

    // Przechowywanie danych o produkcie
    $_SESSION[$nr . '_0'] = $nr;
    $_SESSION[$nr . '_1'] = $productId;
    $_SESSION[$nr . '_2'] = $quantity;
    $_SESSION[$nr . '_3'] = $cena_netto_calkowita;
    $_SESSION[$nr . '_4'] = $vat;
    $_SESSION[$nr . '_5'] = $cena_brutto_calkowita;
    $_SESSION[$nr . '_6'] = time();

    echo '<p style="color: green;">Produkt został dodany do koszyka!</p>';
}

function removeFromCart($nr) {
    // Usunięcie danych o produkcie
...
function showCart() {
    ...
    for ($i = 1; $i <= $_SESSION['count']; $i++) {
        if (isset($_SESSION[$i . '_1'])) {
            $productId = $_SESSION[$i . '_1'];
            $quantity = $_SESSION[$i . '_2'];
            $cena_netto_calkowita = $_SESSION[$i . '_3'];
            $vat = $_SESSION[$i . '_4'];
            $cena_brutto_calkowita = $_SESSION[$i . '_5'];
            $data = date("Y-m-d H:i:s", $_SESSION[$i . '_6']);
            
            $total += $cena_brutto_calkowita;

            echo "<tr>
                <td>{$productId}</td>
                <td>{$quantity}</td>
                <td>" . number_format($cena_netto_calkowita, 2) . " zł</td>
                <td>{$vat}%</td>
                <td>" . number_format($cena_brutto_calkowita, 2) . " zł</td>
                <td>{$data}</td>
                <td><a href='?action=remove_from_cart&id={$i}' class='button'>Usuń</a></td>
            </tr>";
        }
    }

    echo "<tr><td colspan='4'><b>Suma:</b></td><td><b>" . number_format($total, 2) . " zł</b></td><td></td></tr>";
    echo "</table>";
}
...
if (isset($_POST['add_to_cart'])) {
    ...
    addToCart($id_prod, $ile_sztuk, $cena_netto, $vat);
}

// Usuwanie produktu z koszyka
if (isset($_POST['remove'])) {
    $id_prod = $_POST['remove_id'];
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        ...
        case 'add_to_cart':
			if (isset($_GET['id'], $_GET['quantity'], $_GET['cena_netto'], $_GET['vat'])) {
				addToCart(
					(int)$_GET['id'], 
					(int)$_GET['quantity'], 
					(float)$_GET['cena_netto'], 
					(float)$_GET['vat']
				);
			header("Location: admin.php?action=cart");
        exit;
    }
    break;
    ...
    echo '<p>Wybierz jedną z opcji powyżej, aby zarządzać podstronami, kategoriami, produktami lub koszykiem.</p>';
}
